    ...
    <div class="bg-neutral-900">
      ...
      >
        <h6 :class="{ hidden: !loggedIn }">
          Hi Shrek Potvin
          <div v-if="currentUserName">Current user: {{ currentUserName }}</div>
          <div>
            // v-for="user in users" :key="user.id" - Va dans le Div en haut
            <ScaleButton @set-current-user="setCurrentUser" :user="user"
              >Test</ScaleButton
            >
          </div>
        </h6>
...
export default {
  ...
  components: { ScaleButton: ScaleButton },
